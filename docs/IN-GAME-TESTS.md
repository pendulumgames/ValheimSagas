...
3. Press Left Ctrl+Insert in game. Expect a short confirmation, with no credential in the game log. Paste into website Login within two minutes. Confirm your identity. Pressing it again revokes the previous token. Left Ctrl+End revokes the active token. Verify clipboard cleanup preserves an unrelated newly copied value. Customize shortcuts in Website Login if another mod uses those keys.
