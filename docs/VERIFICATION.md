...
## 0.3.33 - Recorded adventures and independent loading

Implemented independent world/map bootstrap, bounded history caching with fresh privacy checks, feed/version status and mixed-package warnings; per-world since-last-visit recaps, eight-boss trophy hall, filtered Viking comparisons and recorded chapter map locations. Existing scalable map renderer retained. Moments integration deferred pending the documented identity/consent/export contract; only SAGAS-INTEGRATION-PLAN.md added in the Moments repository, no code changed. Excluded host diagnostics, epithets and Steam achievements.

Verification: full build passed with zero errors and two existing MSB3277 warnings: 2,864 Core assertions, 83 synthetic HTTP lore, 22,017 terrain, 1,557 portrait, 13 optional-adapter and 237 installed-assembly metadata checks. All browser suites passed individually, including two complete acceptance runs with 96 grounding combinations, independent-loading/privacy races, trophies/comparisons, authentication, map and optional-mod coverage. Acceptance network-error mocking now uses one phase-controlled interceptor to avoid teardown races; no application failure was hidden. Synthetic 10,000-event history benchmark: first all-history response 318.48 ms, warm median 81.88 ms; no previous-version comparison inferred. Synthetic fully exposed 76,648-cell world with 1,000 pins, 40,000 SLS zones and 5,000 heat events: redraw median 8.15 ms / max 24.6 ms. These are local development measurements, not dedicated-server or gameplay verification.

Exact next steps: install the complete 0.3.33 package after closing Valheim, preserving configuration/data; coordinate matching server/client versions, reload the website, and follow docs/IN-GAME-TESTS.md. The live host was previously serving stale 0.3.27 web files; update its whole web folder before comparing responsiveness. No live deployment, provider calls or game/save modifications performed. Package: artifacts/ValheimSagas-0.3.33.zip. Moments plan: C:/repository/ValheimMoments/SAGAS-INTEGRATION-PLAN.md.


## 0.3.34 illustrated trophies and comparison

Build passed with zero errors and two existing MSB3277 reference warnings. Checks: 2,868 Core assertions; 83 synthetic HTTP lore checks; 22,017 terrain checks; 1,557 portrait checks; 13 optional-adapter checks; 237 installed-assembly metadata checks. New Core cases cover highest-star selection, untimed high-star encounters, timed ties, filtered-period unlock persistence and private profile exclusion.

Browser coverage includes eight locally served original illustrations, SVG frames and mist, lifetime unlock versus period records, one-time observed reveal, reduced motion, comparison column geometry and 1500/390/320 px layouts. Screenshots in TestingOutputs are explicitly synthetic development telemetry, not gameplay. Original source art is interpreted, not an official game asset; Kall uses the user-approved symbolic frozen monument.

No game capture/network hooks changed. No in-game, dedicated-server or live-provider validation occurred for this update. Animated saga journeys are not implemented in 0.3.34.

## 0.3.35 saga journeys

- Full build: zero errors, two existing MSB3277 assembly-reference warnings. 2,888 Core assertions (including journey consent, chronological gaps, world/owner scope, bounded paging, and extended signed-coordinate map coverage); 83 synthetic HTTP lore checks; 22,017 terrain checks; 1,557 portrait checks; 13 optional adapter present/absent checks; 237 installed-assembly metadata checks. No game code execution is implied.
- Journey browser checks: lazy independent index, real synthetic HTTP detail, decoded local art, map relocation without a second terrain renderer, replay/pause/restart/scrub, desktop/mobile at 390 and 320 pixels, reduced motion, scope-change races, unavailable chapter, and fresh-tab cross-world links. Source and screenshot labels identify synthetic evidence.
- Existing browser suite passed through atlas controls. Directory test assumed a database return order; it now selects fixture Vikings by stable ID. Directory plus all remaining roster/cluster/Jewelcrafting/optional-mod suites passed afterward. No application behavior was relaxed for that test.
- Synthetic maximum journey benchmark at 2560 x 1440: 100 moments plus 1,000 map icons, steady view with detail loading paused to isolate overlay cost. Median 0.9 ms, p95 1.2 ms, max 1.4 ms; zero terrain redraws during 750 ms of explicit replay. Normal background terrain/detail arrivals can still redraw the base map. This is development-browser CPU timing, not end-user GPU or game-frame acceptance.
- Existing full-world stress check: 76,648 tiles, 1,000 pins, 40,000 SLS zones and 5,000 events; worst sampled full redraw 20.6 ms. The production terrain renderer was retained.
- Review screenshots in TestingOutputs use a clean fixture populated from read-only exports of previously captured terrain, with synthetic story/events. An old copied database's runtime/culture-dependent index issue was isolated; no production migration change resulted. No live data, game saves or provider credentials were touched.

Logs: .dev/build0335-final.log, .dev/web0335.log, .dev/web0335-rest.log, .dev/journeys0335-final.log, .dev/journey-performance0335.json, .dev/map-stress0335.json. Real dedicated-server, Steam-overlay and in-game acceptance remain pending.


## 0.3.35 six-moment journey refinement

Cap chapter journeys at six curated highlights after privacy/exploration filtering. Prefer boss/Nemesis victories, rare discoveries, notable fights and biome/spatial variety; penalize repeated same-item/creature records. Restore chronological order and preserve hidden-record, player, time and distance breaks across omitted moments. Full chapter/evidence records remain unchanged. Crowded moment markers use collision-tested screen positions and true-location spokes, with redundant floating text removed. Replay dock sits above navigation on desktop/mobile. Opening a chapter fits after layout settles with control/marker margins. Moment art follows recorded biome and boss; boss art clears on non-boss moments, with boss-biome fallback if biome metadata is absent.

Verification: full build zero errors/two existing reference warnings, 2892 Core assertions,83 lore,22017 terrain,1557 portrait,13 adapter,237 installed metadata checks. Browser suite covers six coincident markers, fit/centering, desktop/mobile dock clearance, biome/boss artwork changes, playback and privacy races. Real Test fixture copy checked under Unity Mono: all three chapters return six shared explored highlights. No Test database edits this follow-up. Adversarial browser test with100 injected moments (above real six cap)/1000 pins: overlay p953.6ms,max4.5ms,zero terrain redraws in750ms. Reviewed generated desktop screenshot. No in-game verification claimed.


## 0.3.36 story-scene verification

- Full build passed with zero errors and two existing MSB3277 reference warnings: 2,896 Core assertions,94 lore checks using synthetic HTTP,22,017 terrain,1,557 portrait,13 optional-adapter and237 installed-assembly metadata checks.
- All browser suites passed across .dev/web0336.log and .dev/web0336-rest.log. The first run stopped at the login harness's default fixture-file path; remaining suites passed after supplying SAGAS_LOGIN_FIXTURE for the active isolated DevHost. Scene-specific tests verify passage/title changes, full-chapter consistency, replay updates, larger boss markers, matching/cleared artwork, named chapter list and mobile layout. Existing privacy races, map fit, coincident markers and control clearance passed.
- Persistence and hostile-output tests cover valid request-local evidence references, duplicate/unknown/out-of-order/actual-ID references, oversized scene counts, HTML rejection and private identifier/coordinate omission. No real OpenRouter requests or credentials used. Scene generation stays in the existing single queued request. Legacy chapter IDs and stored text are unchanged.
- OpenRouter API documentation checked September24: https://openrouter.ai/docs/guides/features/structured-outputs and https://openrouter.ai/docs/api/api-reference/chat/send-chat-completion-request . Strict schema routing is model-dependent, so the default free/preset routes retain application-side JSON validation rather than requiring a different model/provider. The output cap is2,000 tokens; paid/free controls remain unchanged.
- Adversarial browser replay with100 injected moments (above production's six limit),1,000 icons and2560x1440 viewport: median1.9ms,p953.7ms,max4.2ms,zero terrain redraws in750ms. Development-browser timings, not game-frame acceptance.
- docs/screenshots/saga.png is a real working-website capture with deliberately illustrative story/events, anonymous example identity and example clock over an isolated copy of playtest terrain. These fixture records/terrain are not packaged. No actual provider-generated chapter, multiplayer server or in-game acceptance claimed this turn.
- Release validation accepts the exact allowlisted archive and rejects11 malformed archives, including an injected seeded-story database. Core/plugin/website version0.3.36; game and optional-mod DLLs, secrets, configs, saves, databases and DevHost are excluded.
