...
internal sealed partial class RuntimeArt {
 ...
 public void PumpPortrait(Player? player,bool permitted,string world){
  ...
  try{if(!capture.MoveNext()){captureTiming.Add(Time.frameCount,timer.Elapsed.TotalMilliseconds);timer.Restart();CancelCapture();Debug.Log("Sagas staged portrait: "+captureSteps+" steps, max main-thread capture work/frame="+captureTiming.Maximum.ToString("F1")+" ms, elapsed="+(Time.realtimeSinceStartup-captureStarted).ToString("F2")+" s; image processing continues in background.");}}
  ...
  finally{captureSteps++;captureTiming.Add(Time.frameCount,timer.Elapsed.TotalMilliseconds);}
 }
 ...
 IEnumerable<object?> BuildPortrait(Player player,string signature){
  ...
  try{
   uint usedLayers=0;var existing=UnityEngine.Object.FindObjectsByType<Renderer>(FindObjectsSortMode.None);
   ...
   var frozen=FreezePoses(player);var frozenPosition=player.transform.position;var frozenYaw=player.transform.eulerAngles.y;
   ...
   yield return null;
   studio=PortraitLighting.CreateStudioReflection();yield return null;
   ...
   for(int stage=0;stage<4;stage++){
    ...
    yield return null;
    pair=PortraitReadbackPair.Submit(camera,root,width,studio,ms=>captureTiming.Add(Time.frameCount,ms));
