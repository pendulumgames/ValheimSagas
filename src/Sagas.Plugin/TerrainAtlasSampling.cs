...
internal static class TerrainAtlasSampling {
 internal static byte[] Tile(byte[] atlas,int size,float worldExtent,int cx,int cz){
  ...
  return Tile(i=>atlas[i],size,worldExtent,cx,cz);
 }
 internal static byte[] Tile(Func<int,byte> pixel,int size,float worldExtent,int cx,int cz){
  ...
  for(int z=0;z<64;z++)for(int x=0;x<64;x++){
   ...
   for(int c=0;c<4;c++){
    float bottom=pixel((z0*size+x0)*4+c)*(1-fx)+pixel((z0*size+x1)*4+c)*fx;
    float top=pixel((z1*size+x0)*4+c)*(1-fx)+pixel((z1*size+x1)*4+c)*fx;
