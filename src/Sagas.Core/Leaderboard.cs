...
public sealed partial class SagaService {
 static object IntegrationSummary(IEnumerable<PlayerSnapshot> source,bool nemesis){var players=source.ToArray();return new{epicLoot=players.Any(p=>p.EpicLootInstalled),jewelcrafting=players.Any(p=>p.JewelcraftingInstalled),nemesis};}
 static readonly (string Key,string Name,int Order,string Biome)[] KnownBosses=BossCatalog.Bosses;
 ...
 public object Leaderboard(string world,TimeWindow window,HashSet<string>? selected=null){
  ...
  var bossEvents=events.Where(e=>e.Kind=="kill"&&e.Boss&&!e.NemesisBoss&&!BossCatalog.IntermediatePhase(e.Prefab)).Select(e=>{
   ...
   return new {Event=e,Row=new LeaderboardBossKill{BossKey=identity.Key,BossName=identity.Name,Order=identity.Order,Utc=e.Utc,DurationSeconds=!BossCatalog.FinalNorthPhase(e.Prefab)&&e.DurationSeconds.HasValue&&e.DurationSeconds.Value>0&&!double.IsNaN(e.DurationSeconds.Value)&&!double.IsInfinity(e.DurationSeconds.Value)?e.DurationSeconds:null,Stars=e.Stars,Team=team,Finisher=players.ContainsKey(e.PlayerId)?Person(e.PlayerId):null}};
  ...
  var nemesisEvents=events.Where(e=>e.Kind=="kill"&&e.NemesisBoss).GroupBy(e=>e.Id,StringComparer.Ordinal).Select(g=>g.First()).ToArray();
  var nemesisBossKills=Rank(nemesisEvents.SelectMany(e=>e.Contributors.Concat(new[]{e.PlayerId}).Where(players.ContainsKey).Distinct(StringComparer.Ordinal)).GroupBy(id=>id,StringComparer.Ordinal).Select(g=>new KeyValuePair<string,long>(g.Key,g.LongCount())));
