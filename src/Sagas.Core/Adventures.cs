...
public sealed partial class SagaService {
 ...
 public object Adventures(string world,TimeWindow window,DateTime? since=null,HashSet<string>? selected=null){
  ...
  var trophyKeys=KnownBosses.Select(b=>(Key:b.Key,Name:b.Name,Order:(int?)b.Order)).Concat(records.Where(b=>!b.Order.HasValue).Select(b=>(Key:b.BossKey,Name:b.BossName,Order:b.Order))).GroupBy(b=>b.Key,StringComparer.OrdinalIgnoreCase).Select(g=>g.First());
  var trophies=trophyKeys.Select(b=>{var rows=records.Where(r=>string.Equals(r.BossKey,b.Key,StringComparison.OrdinalIgnoreCase)).ToArray();return new{key=b.Key,name=b.Name,order=b.Order,kills=rows.Length,uniquePlayers=rows.SelectMany(r=>r.Team).Select(p=>p.PlayerId).Distinct().Count(),first=rows.FirstOrDefault(),fastest=rows.Where(r=>r.DurationSeconds.HasValue).OrderBy(r=>r.DurationSeconds).ThenBy(r=>r.Utc).FirstOrDefault()};}).OrderBy(b=>b.order??int.MaxValue).ThenBy(b=>b.key,StringComparer.Ordinal).ToArray();
...
public sealed class AdventureMoment {
 public string Id{get;set;}="";public DateTime Utc{get;set;}public string Kind{get;set;}="";public string Name{get;set;}="";public string PlayerId{get;set;}="";public string PlayerName{get;set;}="";public int Stars{get;set;}public int Amount{get;set;}public string Rarity{get;set;}="";public string RarityColor{get;set;}="";public bool Boss{get;set;}public bool NemesisBoss{get;set;}public LeaderboardPerson[] Team{get;set;}=new LeaderboardPerson[0];public float? X{get;set;}public float? Z{get;set;}
}
