...
public sealed partial class SagaStore {
 public LoreContext NarrativeContext(string world,string? character=null) {
  lock(gate) {
   ...
   foreach(var e in history.Where(e=>e.Kind=="kill"&&e.Boss&&(character==null?shared.ContainsKey(e.PlayerId)||e.Contributors.Any(shared.ContainsKey):e.PlayerId==character||e.Contributors.Contains(character))).OrderByDescending(e=>e.Utc).Take(8)) {
