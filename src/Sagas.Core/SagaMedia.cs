...
public sealed partial class SagaStore {
 public void Media(MediaUpload m){lock(gate){var c=db.GetCollection("media");c.Upsert(new BsonDocument{{"_id",Key(m.World,m.PlayerId,m.Id)},{"world",m.World},{"player",m.PlayerId},{"kind",m.Kind},{"utc",DateTime.UtcNow},{"png",m.Png}});
  ...
  var old=c.Find(Query.EQ("world",m.World)).Where(d=>d["player"].AsString==m.PlayerId&&d["kind"].AsString==m.Kind).OrderByDescending(d=>d["utc"].AsDateTime).Skip(m.Kind=="portrait"?2:m.Kind=="map-icon"?128:64).ToArray();foreach(var d in old)c.Delete(d["_id"]);
 }}
 ...
 internal bool IsMapIcon(string world,string player,string id){lock(gate)return db.GetCollection("media").FindById(Key(world,player,id))?["kind"].AsString=="map-icon";}
}
public sealed partial class SagaService {
 ...
 public static bool ValidMedia(MediaUpload m){if(m==null||!TextValid(m.World,100,true)||!TextValid(m.PlayerId,100,true)||m.Id==""||!MediaId(m.Id)||(m.Kind!="icon"&&m.Kind!="portrait"&&m.Kind!="map-icon")||m.Png==null||m.Png.Length<33||m.Png.Length>(m.Kind=="portrait"?MaximumPortraitBytes:MaximumIconBytes))return false;
