...
public sealed class WirePart { public string Id=""; public int Lane,Index,Count; public byte[] Data=Array.Empty<byte>(); }
public sealed class WireTransfer {
 public const int Chunk=2400, Maximum=128000;
 sealed class Entry { public string Id=""; public double Started; public byte[][] Parts=Array.Empty<byte[]>(); }
 ...
 public byte[]? Accept(string owner,WirePart p,double now){
  ...
  owner=owner+":"+p.Lane;
  if(!entries.TryGetValue(owner,out var e)||e.Id!=p.Id){if(p.Index!=0||entries.Count>=128&&!entries.ContainsKey(owner))return null;entries[owner]=e=new Entry{Id=p.Id,Started=now,Parts=new byte[p.Count][]};}
  if(e.Parts.Length!=p.Count)return null;e.Parts[p.Index]=p.Data;
  if(e.Parts.Any(x=>x==null))return null;entries.Remove(owner);
  int length=e.Parts.Sum(x=>x.Length);if(length>Maximum)return null;
  var result=new byte[length];int offset=0;foreach(var part in e.Parts){Array.Copy(part,0,result,offset,part.Length);offset+=part.Length;}return result;
 }
}
