...
public sealed partial class SagaService : IDisposable {
 ...
 public object State(string world,TimeWindow window,HashSet<string>? selected=null) {
  ...
  SanitizePlayers(players,sls);
  var displayed=events.OrderByDescending(e=>e.Utc).Take(5000).Select(Copy).ToArray();
  store.HideUnknownEventCoordinates(world,shared,displayed);
  foreach(var e in displayed)e.Contributors=e.Contributors.Where(id=>!string.IsNullOrEmpty(id)&&visible.Contains(id)).Distinct(StringComparer.Ordinal).ToList();
  var kills=events.Where(e=>e.Kind=="kill").ToList();var loot=events.Where(e=>e.Kind=="drop"||e.Kind=="collect"||e.Kind=="pickup").ToList();
  var chapters=store.Chapters(world).Where(c=>GeneratedChapter(c)&&visible.Contains(c.PlayerId)&&c.Participants.All(p=>p.PlayerId!=""&&visible.Contains(p.PlayerId))&&c.MapParticipants.All(id=>id!=""&&visible.Contains(id)&&shared.Contains(id))).Select(WithoutTemplateBio).ToArray();
  return new {bossCatalog=KnownBosses.Select(b=>new{key=b.Key,name=b.Name,order=b.Order}).ToArray(),sls=SlsSummary(sls),server=new{name=options.ServerName,address=options.ServerAddress},world,worlds=store.Worlds(),worldNames=store.WorldNames(),trackingSince=store.TrackingSince,statisticsSince=store.StatisticsSince,detailRetentionDays=options.RetentionDays,partialHistory=window.From<store.TrackingSince||(store.StatisticsSince.HasValue&&window.From<store.StatisticsSince.Value),from=window.From,to=window.To,players,events=displayed,eventCount=events.Count,eventsTruncated=events.Count>5000,chapters,synthetic=options.Synthetic,
   stats=new{integrations=IntegrationSummary(players.Where(p=>p.ShareProfile&&(selected==null||selected.Contains(p.PlayerId))),sls.Installed&&sls.NemesisEnabled),equippedGems=players.Where(p=>p.ShareProfile&&(selected==null||selected.Contains(p.PlayerId))).Sum(p=>p.Gear.Sum(g=>g.Sockets.Count(x=>x.Prefab!=""))),nemesisBossKills=history.Where(e=>e.Kind=="kill"&&e.NemesisBoss&&window.Contains(e.Utc)).Count(e=>e.Contributors.Concat(new[]{e.PlayerId}).Any(id=>id!=""&&visible.Contains(id)&&(selected==null||selected.Contains(id)))),bounties=events.Count(e=>e.Kind=="bounty"),deaths=events.Count(e=>e.Kind=="death"),bossKills=kills.Count(e=>e.Boss),highestStars=kills.Count==0?(int?)null:kills.Max(e=>e.Stars),creatureTypes=kills.Select(e=>string.IsNullOrEmpty(e.Prefab)?e.Name:e.Prefab).Distinct(StringComparer.OrdinalIgnoreCase).Count(),rarityCollected=loot.Where(e=>e.Kind=="collect").GroupBy(e=>LeaderboardRarity(e.Rarity)).ToDictionary(g=>g.Key,g=>g.Sum(e=>(long)e.Amount)),kills=kills.Count,dropped=loot.Where(e=>e.Kind=="drop").Sum(e=>(long)e.Amount),collected=loot.Where(e=>e.Kind=="collect").Sum(e=>(long)e.Amount),unknownPickups=loot.Where(e=>e.Kind=="pickup").Sum(e=>(long)e.Amount),stars=kills.GroupBy(e=>e.Stars).ToDictionary(g=>g.Key.ToString(),g=>g.Count()),
...
var q=c.Request.QueryString;string world=string.IsNullOrWhiteSpace(q["world"])?options.World:q["world"]!;if(world.Length>100){await Respond(c,400,new{error="Invalid world."});return;}HashSet<string>? ids=string.IsNullOrWhiteSpace(q["players"])?null:new HashSet<string>(q["players"]!.Split(',').Where(x=>x.Length>0));if(ids!=null&&ids.Count>100){await Respond(c,400,new{error="Too many players."});return;}
   try{if(path=="/api/session")await Respond(c,200,LoginSession(world,identity));else if(path.StartsWith("/api/media/",StringComparison.Ordinal))await ServeMedia(c,world,q["player"]??"",path.Substring(11));else if(path=="/api/health")await Respond(c,200,Health());else if(path=="/api/version")await Respond(c,200,VersionInfo());else if(path=="/api/world-state")await Respond(c,200,WorldState(world));else if(path=="/api/state")await Respond(c,200,State(world,TimeWindow.Parse(q["range"],q["from"],q["to"],DateTime.UtcNow),ids));else if(path=="/api/adventures")await Respond(c,200,Adventures(world,TimeWindow.Parse(q["range"],q["from"],q["to"],DateTime.UtcNow),AdventureSince(q["since"]),ids));else if(path=="/api/saga-journeys")await Respond(c,200,SagaJourneys(world,ids,q["before"]??""));else if(path=="/api/saga-journey")await Respond(c,200,SagaJourney(world,q["chapter"]??"",ids));else if(path=="/api/saga-map")await Respond(c,200,ChapterMap(world,q["chapter"]??"",ids));else if(path=="/api/map-icons")await Respond(c,200,MapIcons(world,ids));else if(path=="/api/world-clock")await Respond(c,200,new{clock=Clock(world),serverUtc=DateTime.UtcNow});else if(path=="/api/map-stream")await Respond(c,200,MapStream(world,ids,q["scope"],q["cursor"]));else if(path=="/api/map-meta")await Respond(c,200,MapMeta(world,ids));else if(path=="/api/map-detail")await Respond(c,200,MapDetail(world,ids,q["keys"]??"",q["side"]==null?64:int.TryParse(q["side"],out var mapSide)?mapSide:0));else if(path=="/api/map")await Respond(c,200,Map(world,ids));else if(path=="/api/leaderboard")await Respond(c,200,Leaderboard(world,TimeWindow.Parse(q["range"],q["from"],q["to"],DateTime.UtcNow),ids));else if(path=="/api/server-saga")await Respond(c,200,ServerSaga(world));else await Respond(c,404,new{error="Not found"});}catch(ArgumentException e){await Respond(c,400,new{error=e.Message});}return;
  }
 ...
 static Dictionary<string,string> BuildStaticFiles(){
  var files=new Dictionary<string,string>(StringComparer.Ordinal){{"/","index.html"},{"/index.html","index.html"},{"/app.js","app.js"},{"/journey.js","journey.js"},{"/journey.css","journey.css"},{"/styles.css","styles.css"},{"/style.css","style.css"},{"/favicon.svg","favicon.svg"}};
  ...
  foreach(var boss in new[]{"eikthyr","elder","bonemass","moder","yagluth","queen","fader","kall-monument"}){var file="bosses/"+boss+".webp";files.Add("/"+file,file);}
  return files;
 }
