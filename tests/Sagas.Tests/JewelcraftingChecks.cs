...
static class JewelcraftingChecks {
 public static async Task Run(string root,Action<bool,string> check){
  ...
  var player=new PlayerSnapshot{World="jewel",PlayerId="p",Name="Synthetic Jeweler",ShareProfile=true,Gear=new(){gear}};
  ...
  using(var service=new SagaService(options)){
   ...
   var state=JObject.Parse(await client.GetStringAsync("api/state"));check(state["players"]![0]!["gear"]!.Count()==0,"Private sockets are not exposed in state");
  }
 }
}
