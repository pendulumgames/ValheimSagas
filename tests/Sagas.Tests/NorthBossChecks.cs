...
static class NorthBossChecks {
 ...
 public static void Run(string root,Action<bool,string> check){
  ...
  using(var s=new SagaService(new(){DataDirectory=path,ListenPrefix="",World="north"})){
   foreach(var id in new[]{"champion","team","private","phase-only"})s.Store.Player(new(){World="north",PlayerId=id,Name=id,ShareProfile=id!="private",ProgressionTier=999,ProgressionBoss="Spoofed",LastAchievementUtc=now.AddYears(1)});
