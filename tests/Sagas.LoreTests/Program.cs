...
Check(!generated.Text.StartsWith("Fictional embellishment:") && generated.Text.Length >= 40, "New chapters store prose without injected disclaimer");
Check(generated.Id == local.Id, "Prompt revision does not invalidate existing chapter cache identities");
var body = JObject.Parse(handler.Bodies.Last());
var writingPrompt = (string)body["messages"]![0]!["content"]!;
Check(writingPrompt.Contains("one central thread") && writingPrompt.Contains("omit routine loot lists") && writingPrompt.Contains("never invent achievements"), "Narrative prompt selects an arc while preserving factual constraints");
