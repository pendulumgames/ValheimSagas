...
(async () => {
 ...
 try {
  ...
  await page.waitForFunction(() => state.data?.stats&&!state.busy&&!state.mapLoading&&!mapStreamBusy);
  ...
  await page.evaluate(async()=>{state.busy=true;worldDue=Infinity;appliedPresence=++presenceSerial;while(mapStreamBusy)await new Promise(r=>setTimeout(r,10));clearTimeout(mapStreamTimer);});
...
await page.evaluate(()=>drawMap());
  ...
  await page.locator('.masthead nav a[href="#world"]').click();
  // Keep one interceptor across these phases: tearing routing down while profile requests
  // are still completing can race Playwright's detached interception-pattern updates.
  let failurePhase='storage';
  await page.route('**/api/state?**',async route=>{
   if(failurePhase==='http')return route.fulfill({status:500,contentType:'application/json',body:JSON.stringify({error:'Recorded data is temporarily unavailable',errorId:'fixture-error-011'})});
   const phase=failurePhase,response=await route.fetch();const json=await response.json();
   if(phase==='storage')json.health={storageError:'SyntheticStorageFailure',errorId:'fixture-health-011'};
   await route.fulfill({response,json});
  });
  await revealAtlas(page,'filters');await page.locator('#filters button[type=submit]').click();
  ...
  failurePhase='http';
  const failedStatistics=page.waitForResponse(r=>r.url().includes('/api/state?')&&!new URL(r.url()).searchParams.has('players')&&r.status()===500);
  await revealAtlas(page,'filters');await page.locator('#filters button[type=submit]').click();
  assert.equal((await failedStatistics).status(),500,'Synthetic HTTP500 reaches the actual overview statistics request');
...
await publicPage.route('**/api/state?**',route=>route.fulfill({status:401,contentType:'application/json',body:JSON.stringify({error:'Token now required'})}));await publicPage.evaluate(()=>refresh());await publicPage.waitForFunction(()=>document.querySelector('#accessButton').textContent==='Login');assert.equal(await publicPage.locator('#accessButton').isEnabled(),true,'Public-to-private change restores login button');assert.equal(await publicPage.evaluate(()=>state.publicViewing),false,'401 stops unauthenticated public polling');await publicPage.unrouteAll({behavior:'wait'});await publicPage.close();
  ...
  await portraitPage.evaluate(()=>{state.busy=true;worldDue=Infinity;appliedPresence=++presenceSerial;const p=state.data.players.find(p=>p.playerId==='fixture-0');p.profileBiome='DeepNorth';renderProfile();});
