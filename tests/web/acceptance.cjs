...
await page.evaluate(()=>drawMap());
  ...
  await page.locator('.masthead nav a[href="#profile"]').click();
  ...
  await page.locator('#world').selectOption('synthetic-midgard');await page.waitForFunction(()=>state.data?.world==='synthetic-midgard');await page.locator('#leaderboardRange').selectOption('all');await page.locator('#leaderboardFilters button').click();await page.waitForFunction(()=>document.querySelector('#leaderboardResults').textContent.includes('No gold snapshots'));assert((await page.locator('#leaderboardResults').textContent()).includes('No measured boss fights'),'Missing durations not ranked as zero');await page.locator('.masthead nav a[href="#profile"]').click();await page.locator('#characterSearch').fill('Astrid');await page.keyboard.press('ArrowDown');await page.keyboard.press('Enter');
  ...
  await page.locator('.masthead nav a[href="#profile"]').click();await page.locator('[data-profile-tab="saga"]').click();await page.screenshot({path:path.join(output,'viking-saga.png')});await page.locator('[data-profile-tab="character"]').click();
...
await publicPage.route('**/api/state?**',route=>route.fulfill({status:401,contentType:'application/json',body:JSON.stringify({error:'Token now required'})}));await publicPage.evaluate(()=>refresh());await publicPage.waitForFunction(()=>document.querySelector('#accessButton').textContent==='Login');assert.equal(await publicPage.locator('#accessButton').isEnabled(),true,'Public-to-private change restores login button');assert.equal(await publicPage.evaluate(()=>state.publicViewing),false,'401 stops unauthenticated public polling');await publicPage.unrouteAll({behavior:'wait'});await publicPage.close();
  ...
  await portraitPage.locator('[data-backdrop="full"]').click();
  ...
  await portraitPage.locator('[data-profile-tab="saga"]').click();assert.equal(await portraitPage.evaluate(()=>document.body.classList.contains('scenic-profile')),true,'Full screen scenery remains on Saga tab');assert.equal(await portraitPage.locator('#backdropControls').isVisible(),true,'Saga keeps backdrop controls available');await portraitPage.screenshot({path:path.join(output,'backdrop-full-saga.png')});
  ...
  await portraitPage.locator('.masthead nav a[href="#profile"]').click();
  ...
  await portraitPage.locator('[data-backdrop="boxed"]').click();assert.equal(await portraitPage.evaluate(()=>document.body.classList.contains('scenic-profile')),false,'Boxed comparison can be restored');assert.equal(await portraitPage.locator('[data-backdrop="boxed"]').getAttribute('aria-pressed'),'true');await portraitPage.screenshot({path:path.join(output,'backdrop-boxed-high-resolution.png')});
  ...
  await portraitPage.locator('[data-backdrop="scroll"]').click();
  ...
  await portraitPage.locator('[data-backdrop="full"]').click();await portraitPage.evaluate(()=>scrollTo({top:200,behavior:'instant'}));assert.equal(await portraitPage.locator('#profileBackdrop').evaluate(e=>e.getBoundingClientRect().top),0,'Full screen retains its fixed behavior while scrolling');
  await portraitPage.locator('[data-backdrop="scroll"]').click();await portraitPage.reload();await portraitPage.waitForFunction(()=>state.profileData?.stats&&!state.busy);
  ...
  await portraitPage.locator('.masthead nav a[href="#profile"]').click();await portraitPage.waitForFunction(()=>document.body.dataset.page==='profile');await portraitPage.evaluate(()=>{state.busy=true;const p=state.data.players.find(p=>p.playerId===state.profile);p.profileBiome='';renderProfile();});assert.equal(await portraitPage.locator('#profileBackdrop').isVisible(),false,'Withheld biome cannot retain either artwork set');assert.equal(await portraitPage.locator('#loadout').evaluate(e=>e.style.getPropertyValue('--biome-art')),'none','Consent revocation clears boxed image too');
