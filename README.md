...
The atlas has separate **Discovered game icons** (on) and **Player-added icons** (off) layers. Game locations, boss markers, beds and death/event markers come from each client's existing map, only on personally explored, shared terrain. They do not reveal undiscovered locations. Enable `[Privacy] SharePlayerPins = true` in your own Sagas config to publish your manually placed pins; this is off by default and also requires `ShareMap`. Imported pins owned by another Viking are excluded. The browser's player-icon switch controls display, not sharing consent. Hover/tap an icon for its name, or use **Browse visible icons** to locate one.

Icons load separately from terrain and statistics; their runtime sprites are cached, never bundled as copied game assets. Scans examine at most 16 pins per frame, bounded to 2,048 exported pins per character. Changed snapshots upload in paced 32-pin batches; unchanged snapshots only resync every two minutes for recovery. First icons may take 15-30 seconds plus queued upload time to appear, and partially imported terrain can keep them fog-hidden longer. Pin edits/removals replace the prior set only after every part arrives. The last shared pins remain available offline.

Choose **Sun & moon dial**, **Horizon**, or **None** under **Map icons & daylight**. This uses the host's actual world day and day/night fraction, not your computer's clock. Browser preferences are saved locally; unavailable/stale clock data is labelled. No new mod dependency, external service or port is required. Install the matching update on the host and participating clients.
...
Use the .NET 10 SDK, PowerShell, a legally installed Valheim game and BepInEx 5. Game assemblies are local compile references only.

```powershell
./scripts/build.ps1 -GameManaged 'YOUR_GAME/valheim_Data/Managed' -BepInExCore 'YOUR_PROFILE/BepInEx/core'
./scripts/package.ps1
./scripts/verify-package.ps1
```

The output is `artifacts/ValheimSagas-0.3.8.zip`, with a SHA-256 inventory. The ZIP structure is compatible with Thunderstore and Hexium; uploading is a separate step. Build paths can be supplied explicitly; defaults describe the original development machine.

Core and lore tests do not need a game or API key:

```powershell
dotnet run --project tests/Sagas.Tests -c Release
dotnet run --project tests/Sagas.LoreTests -c Release
```
